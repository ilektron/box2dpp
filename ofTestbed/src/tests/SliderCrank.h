...
public:
    ...
    {
        Test::Step(settings);
        g_debugDraw.DrawString({{5.0f, static_cast<float>(m_textLine)}}, "Keys: (f) toggle friction, (m) toggle motor");
        m_textLine += DRAW_STRING_NEW_LINE;
        float32 torque = m_joint1->GetMotorTorque(settings->hz);
        g_debugDraw.DrawString({{5.0f, static_cast<float>(m_textLine)}}, "Motor Torque = %5.0f", (float)torque);
        m_textLine += DRAW_STRING_NEW_LINE;
    }
