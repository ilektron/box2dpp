...
typedef unsigned int stbtt_uint32_t;
typedef signed int stbtt_int32_t;
#endif

typedef char stbtt__check_size32[sizeof(stbtt_int32_t) == 4 ? 1 : -1];
...
extern void stbtt_GetFontVMetrics(const stbtt_fontinfo *info, int *ascent, int *descent,
                                  int *lineGap);
...
extern void stbtt_GetFontBoundingBox(const stbtt_fontinfo *info, int *x0, int *y0, int *x1,
                                     int *y1);
...
extern int stbtt_GetCodepointBox(const stbtt_fontinfo *info, int codepoint, int *x0, int *y0,
                                 int *x1, int *y1);
...
extern int stbtt_GetGlyphBox(const stbtt_fontinfo *info, int glyph_index, int *x0, int *y0, int *x1,
                             int *y1);
...
extern int stbtt_GetCodepointShape(const stbtt_fontinfo *info, int unicode_codepoint,
...
extern unsigned char *stbtt_GetCodepointBitmap(const stbtt_fontinfo *info, float scale_x,
...
extern unsigned char *stbtt_GetCodepointBitmapSubpixel(const stbtt_fontinfo *info, float scale_x,
...
extern void stbtt_MakeCodepointBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w,
...
extern void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output,
...
extern void stbtt_GetCodepointBitmapBox(const stbtt_fontinfo *font, int codepoint, float scale_x,
...
extern void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint,
...
extern unsigned char *stbtt_GetGlyphBitmap(const stbtt_fontinfo *info, float scale_x, float scale_y,
                                           int glyph, int *width, int *height, int *xoff,
                                           int *yoff);
extern unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, float scale_x,
                                                   float scale_y, float shift_x, float shift_y,
                                                   int glyph, int *width, int *height, int *xoff,
                                                   int *yoff);
extern void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w,
                                  int out_h, int out_stride, float scale_x, float scale_y,
                                  int glyph);
extern void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output,
                                          int out_w, int out_h, int out_stride, float scale_x,
                                          float scale_y, float shift_x, float shift_y, int glyph);
extern void stbtt_GetGlyphBitmapBox(const stbtt_fontinfo *font, int glyph, float scale_x,
                                    float scale_y, int *ix0, int *iy0, int *ix1, int *iy1);
extern void stbtt_GetGlyphBitmapBoxSubpixel(const stbtt_fontinfo *font, int glyph, float scale_x,
...
extern void stbtt_Rasterize(stbtt__bitmap *result, float flatness_in_pixels, stbtt_vertex *vertices,
                            int num_verts, float scale_x, float scale_y, float shift_x,
...
extern const char *stbtt_GetFontNameString(const stbtt_fontinfo *font, int *length, int platformID,
...
#define ttULONG(p) (*(stbtt_uint32_t *)(p))
#define ttLONG(p) (*(stbtt_int32_t *)(p))
...
stbtt_uint32_t ttULONG(const stbtt_uint8 *p)
{
    return (p[0] << 24) + (p[1] << 16) + (p[2] << 8) + p[3];
}
stbtt_int32_t ttLONG(const stbtt_uint8 *p)
{
    return (p[0] << 24) + (p[1] << 16) + (p[2] << 8) + p[3];
}
...
static stbtt_uint32_t stbtt__find_table(stbtt_uint8 *data, stbtt_uint32_t fontstart,
                                        const char *tag)
{
    stbtt_int32_t num_tables = ttUSHORT(data + fontstart + 4);
    stbtt_uint32_t tabledir = fontstart + 12;
    stbtt_int32_t i;
    for (i = 0; i < num_tables; ++i)
    {
        stbtt_uint32_t loc = tabledir + 16 * i;
...
int stbtt_GetFontOffsetForIndex(const unsigned char *font_collection, int index)
{
    ...
    if (stbtt_tag(font_collection, "ttcf"))
    {
        ...
        {
            stbtt_int32_t n = ttLONG(font_collection + 8);
            if (index >= n)
                return -1;
...
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data2, int fontstart)
{
    ...
    stbtt_uint32_t cmap, t;
    stbtt_int32_t i, numTables;
    ...
    {
        stbtt_uint32_t encoding_record = cmap + 4 + 8 * i;
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
    ...
    stbtt_uint32_t index_map = info->index_map;

    stbtt_uint16 format = ttUSHORT(data + index_map + 0);
    if (format == 0)
    {  // apple byte encoding
        stbtt_int32_t bytes = ttUSHORT(data + index_map + 2);
    ...
    {
        stbtt_uint32_t first = ttUSHORT(data + index_map + 6);
        stbtt_uint32_t count = ttUSHORT(data + index_map + 8);
        if ((stbtt_uint32_t)unicode_codepoint >= first &&
            (stbtt_uint32_t)unicode_codepoint < first + count)
    ...
    {  // standard mapping for windows fonts: binary search collection of ranges
        ...
        stbtt_uint32_t endCount = index_map + 14;
        stbtt_uint32_t search = endCount;
    ...
    {
        stbtt_uint32_t ngroups = ttULONG(data + index_map + 12);
        stbtt_int32_t low, high;
        low = 0;
        high = (stbtt_int32_t)ngroups;
        // Binary search the right group.
        while (low < high)
        {
            stbtt_int32_t mid = low + ((high - low) >> 1);  // rounds down, so low <= mid < high
            stbtt_uint32_t start_char = ttULONG(data + index_map + 16 + mid * 12);
            stbtt_uint32_t end_char = ttULONG(data + index_map + 16 + mid * 12 + 4);
            if ((stbtt_uint32_t)unicode_codepoint < start_char)
                high = mid;
            else if ((stbtt_uint32_t)unicode_codepoint > end_char)
                low = mid + 1;
            else
            {
                stbtt_uint32_t start_glyph = ttULONG(data + index_map + 16 + mid * 12 + 8);
                if (format == 12)
...
static void stbtt_setvertex(stbtt_vertex *v, stbtt_uint8 type, stbtt_int32_t x, stbtt_int32_t y,
                            stbtt_int32_t cx, stbtt_int32_t cy)
{
    v->type = type;
    v->x = (stbtt_int16)x;
    v->y = (stbtt_int16)y;
...
static int stbtt__close_shape(stbtt_vertex *vertices, int num_vertices, int was_off, int start_off,
                              stbtt_int32_t sx, stbtt_int32_t sy, stbtt_int32_t scx,
                              stbtt_int32_t scy, stbtt_int32_t cx, stbtt_int32_t cy)
{
    if (start_off)
    {
        if (was_off)
...
int stbtt_GetGlyphShape(const stbtt_fontinfo *info, int glyph_index, stbtt_vertex **pvertices)
{
    ...
    *pvertices = nullptr;
    ...
    if (numberOfContours > 0)
    {
        ...
        stbtt_int32_t ins, i, j = 0, m, n, next_move, was_off = 0, off, start_off = 0;
        stbtt_int32_t x, y, cx, cy, sx, sy, scx, scy;
        stbtt_uint8 *points;
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            if (next_move == i)
            {
                ...
                if (start_off)
                {
                    // if we start off with an off-curve point, then when we need to
                    // find a point on the curve
                    // where we can start, and we need to save some state for when
                    // we wraparound.
                    scx = x;
                    scy = y;
                    if (!(vertices[off + i + 1].type & 1))
                    {
                        ...
                        // on-point curve
                        sx = (x + (stbtt_int32_t)vertices[off + i + 1].x) >> 1;
                        sy = (y + (stbtt_int32_t)vertices[off + i + 1].y) >> 1;
                    }
                    else
                    {
                        // otherwise just use the next point as our start point
                        sx = (stbtt_int32_t)vertices[off + i + 1].x;
                        sy = (stbtt_int32_t)vertices[off + i + 1].y;
...
void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, float scale_x,
                                         float scale_y, float shift_x, float shift_y, int *ix0,
                                         int *iy0, int *ix1, int *iy1)
{
...
static stbtt__active_edge *new_active(stbtt__edge *e, int off_x, float start_point, void *userdata)
{
    stbtt__active_edge *z =
        (stbtt__active_edge *)STBTT_malloc(sizeof(*z), userdata);  // @TODO: make a pool of these!!!
                ...
                if (i < len && j >= 0)
                {
                    ...
                    else
                    {
                        if (i >= 0)  // add antialiasing for x0
                            scanline[i] =
                                scanline[i] +
                                (stbtt_uint8)(((FIX - (x0 & FIXMASK)) * max_weight) >> FIXSHIFT);
                        else
                            i = -1;  // clip
...
stbtt__point *stbtt_FlattenCurves(stbtt_vertex *vertices, int num_verts, float objspace_flatness,
...
unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, float scale_x,
                                            float scale_y, float shift_x, float shift_y, int glyph,
...
void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h,
...
unsigned char *stbtt_GetCodepointBitmapSubpixel(const stbtt_fontinfo *info, float scale_x,
                                                float scale_y, float shift_x, float shift_y,
...
void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w,
                                       int out_h, int out_stride, float scale_x, float scale_y,
...
extern int stbtt_BakeFontBitmap(const unsigned char *data,
...
static stbtt_int32_t stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8 *s1,
                                                                stbtt_int32_t len1,
...
const char *stbtt_GetFontNameString(const stbtt_fontinfo *font, int *length, int platformID,
                                    int encodingID, int languageID, int nameID)
{
    stbtt_int32_t i, count, stringOffset;
    stbtt_uint8 *fc = font->data;
    stbtt_uint32_t offset = font->fontstart;
    stbtt_uint32_t nm = stbtt__find_table(fc, offset, "name");
    if (!nm)
        return nullptr;
...
static int stbtt__matchpair(stbtt_uint8 *fc, stbtt_uint32_t nm, stbtt_uint8 *name,
                            stbtt_int32_t nlen, stbtt_int32_t target_id, stbtt_int32_t next_id)
{
    stbtt_int32_t i;
    stbtt_int32_t count = ttUSHORT(fc + nm + 2);
    stbtt_int32_t stringOffset = nm + ttUSHORT(fc + nm + 4);

    for (i = 0; i < count; ++i)
    {
        stbtt_uint32_t loc = nm + 6 + 12 * i;
        stbtt_int32_t id = ttUSHORT(fc + loc + 6);
        if (id == target_id)
        {
            // find the encoding
            stbtt_int32_t platform = ttUSHORT(fc + loc + 0), encoding = ttUSHORT(fc + loc + 2),
            ...
            {
                stbtt_int32_t slen = ttUSHORT(fc + loc + 8), off = ttUSHORT(fc + loc + 10);

                // check if there's a prefix match
                stbtt_int32_t matchlen = stbtt__CompareUTF8toUTF16_bigendian_prefix(
                ...
                if (matchlen >= 0)
                {
                    ...
                    {
                        stbtt_int32_t slen = ttUSHORT(fc + loc + 12 + 8),
...
static int stbtt__matches(stbtt_uint8 *fc, stbtt_uint32_t offset, stbtt_uint8 *name,
                          stbtt_int32_t flags)
{
    stbtt_int32_t nlen = (stbtt_int32_t)STBTT_strlen((char *)name);
    stbtt_uint32_t nm, hd;
...
int stbtt_FindMatchingFont(const unsigned char *font_collection, const char *name_utf8,
                           stbtt_int32_t flags)
{
    stbtt_int32_t i;
    for (i = 0;; ++i)
    {
        stbtt_int32_t off = stbtt_GetFontOffsetForIndex(font_collection, i);
        if (off < 0)
            return off;
