...
{
    return (!checkScroll || s_state.insideCurrentScroll) && s_state.mx >= x &&
           s_state.mx <= x + w && s_state.my >= y && s_state.my <= y + h;
}
...
bool imguiBeginScrollArea(const char* name, int x, int y, int w, int h, int* scroll)
{
    ...
    AddGfxCmdText(x + AREA_HEADER / 2, y + h - AREA_HEADER / 2 - TEXT_HEIGHT / 2, TEXT_ALIGN_LEFT,
                  name, SetRGBA(255, 255, 255, 128));
...
bool imguiButton(const char* text, bool enabled)
{
    ...
    AddGfxCmdRoundedRect((float)x, (float)y, (float)w, (float)h, (float)BUTTON_HEIGHT / 2 - 1,
                         SetRGBA(128, 128, 128, isActive(id) ? 196 : 96));
    if (enabled)
...
bool imguiItem(const char* text, bool enabled)
{
    ...
    if (isHot(id))
        AddGfxCmdRoundedRect((float)x, (float)y, (float)w, (float)h, 2.0f,
                             SetRGBA(255, 196, 0, isActive(id) ? 196 : 96));

    if (enabled)
        AddGfxCmdText(x + BUTTON_HEIGHT / 2, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2,
                      TEXT_ALIGN_LEFT, text, SetRGBA(255, 255, 255, 200));
    else
        AddGfxCmdText(x + BUTTON_HEIGHT / 2, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2,
                      TEXT_ALIGN_LEFT, text, SetRGBA(128, 128, 128, 200));
...
bool imguiCheck(const char* text, bool checked, bool enabled)
{
    ...
    if (enabled)
        AddGfxCmdText(x + BUTTON_HEIGHT, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2, TEXT_ALIGN_LEFT,
                      text, isHot(id) ? SetRGBA(255, 196, 0, 255) : SetRGBA(255, 255, 255, 200));
    else
        AddGfxCmdText(x + BUTTON_HEIGHT, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2, TEXT_ALIGN_LEFT,
                      text, SetRGBA(128, 128, 128, 200));
...
bool imguiCollapse(const char* text, const char* subtext, bool checked, bool enabled)
{
    ...
    if (subtext)
        AddGfxCmdText(x + w - BUTTON_HEIGHT / 2, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2,
                      TEXT_ALIGN_RIGHT, subtext, SetRGBA(255, 255, 255, 128));
...
void imguiLabel(const char* text)
{
    ...
    AddGfxCmdText(x, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2, TEXT_ALIGN_LEFT, text,
...
void imguiValue(const char* text)
{
    ...
    AddGfxCmdText(x + w - BUTTON_HEIGHT / 2, y + BUTTON_HEIGHT / 2 - TEXT_HEIGHT / 2,
                  TEXT_ALIGN_RIGHT, text, SetRGBA(255, 255, 255, 200));
}
...
bool imguiSlider(const char* text, float* val, float vmin, float vmax, float vinc, bool enabled)
{
    ...
    if (isActive(id))
        AddGfxCmdRoundedRect((float)(x + m), (float)y, (float)SLIDER_MARKER_WIDTH,
                             (float)SLIDER_HEIGHT, 4.0f, SetRGBA(255, 255, 255, 255));
    else
        AddGfxCmdRoundedRect((float)(x + m), (float)y, (float)SLIDER_MARKER_WIDTH,
                             (float)SLIDER_HEIGHT, 4.0f,
                             isHot(id) ? SetRGBA(255, 196, 0, 128) : SetRGBA(255, 255, 255, 64));
