            ...
            for (int32_t j = 0; j < pointCount; ++j)
                {
                    ...
                    vcp->velocityBias = 0.0f;
                    float32 vRel =
                        b2Dot(vc->normal, vB + b2Cross(wB, vcp->rB) - vA - b2Cross(wA, vcp->rA));
            ...
            for (int32_t j = 0; j < pointCount; ++j)
                {
                    ...
                    float32 C = b2Clamp(BAUMGARTE * (separation + LINEAR_SLOP),
                                        -MAX_LINEAR_CORRECTION, 0.0f);
            ...
            for (int32_t j = 0; j < pointCount; ++j)
                {
                    ...
                    // Prevent large corrections and allow slop.
                    float32 C = b2Clamp(TOI_BAUMGARTE * (separation + LINEAR_SLOP),
                                        -MAX_LINEAR_CORRECTION, 0.0f);
