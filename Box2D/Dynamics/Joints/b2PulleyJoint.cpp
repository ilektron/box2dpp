...
void b2PulleyJointDef::Initialize(b2Body* bA, b2Body* bB, const b2Vec<float, 2>& groundA,
                                  const b2Vec<float, 2>& groundB, const b2Vec<float, 2>& anchorA,
                                  const b2Vec<float, 2>& anchorB, float r)
{
    bodyA = bA;
    bodyB = bB;
    ...
    b2Vec<float, 2> dA = anchorA - groundA;
    lengthA = dA.Length();
    b2Vec<float, 2> dB = anchorB - groundB;
    lengthB = dB.Length();
    ratio = r;
...
void b2PulleyJoint::InitVelocityConstraints(const b2SolverData& data)
{
    ...
    if (data.step.warmStarting)
    {
        ...
        // Warm starting.
        b2Vec<float, 2> PA = -(m_impulse)*m_uA;
        b2Vec<float, 2> PB = (-m_ratio * m_impulse) * m_uB;
...
void b2PulleyJoint::SolveVelocityConstraints(const b2SolverData& data)
{
    ...
    b2Vec<float, 2> PA = -impulse * m_uA;
    b2Vec<float, 2> PB = -m_ratio * impulse * m_uB;
    vA += m_invMassA * PA;
    wA += m_invIA * b2Cross(m_rA, PA);
    vB += m_invMassB * PB;
    wB += m_invIB * b2Cross(m_rB, PB);
...
bool b2PulleyJoint::SolvePositionConstraints(const b2SolverData& data)
{
    ...
    // Get the pulley axes.
    b2Vec<float, 2> uA = cA + rA - m_groundAnchorA;
    b2Vec<float, 2> uB = cB + rB - m_groundAnchorB;
    ...
    float linearError = std::abs(C);

    float impulse = -mass * C;

    b2Vec<float, 2> PA = -impulse * uA;
    b2Vec<float, 2> PB = -m_ratio * impulse * uB;
...
b2Vec<float, 2> b2PulleyJoint::GetAnchorA() const
{
    return m_bodyA->GetWorldPoint(m_localAnchorA);
}

b2Vec<float, 2> b2PulleyJoint::GetAnchorB() const
{
    return m_bodyB->GetWorldPoint(m_localAnchorB);
}

b2Vec<float, 2> b2PulleyJoint::GetReactionForce(float inv_dt) const
{
    b2Vec<float, 2> P = m_impulse * m_uB;
    return inv_dt * P;
}
...
b2Vec<float, 2> b2PulleyJoint::GetGroundAnchorA() const
{
    return m_groundAnchorA;
}

b2Vec<float, 2> b2PulleyJoint::GetGroundAnchorB() const
{
...
{
    b2Vec<float, 2> p = m_bodyA->GetWorldPoint(m_localAnchorA);
    b2Vec<float, 2> s = m_groundAnchorA;
    b2Vec<float, 2> d = p - s;
    return d.Length();
}
...
{
    b2Vec<float, 2> p = m_bodyB->GetWorldPoint(m_localAnchorB);
    b2Vec<float, 2> s = m_groundAnchorB;
    b2Vec<float, 2> d = p - s;
    return d.Length();
}
...
void b2PulleyJoint::ShiftOrigin(const b2Vec<float, 2>& newOrigin)
{
